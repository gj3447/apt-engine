...
- The apt-engine engineering graph — `AptEngine`, `AptPhaseV27`, and the
  ...
  kg-neo4j **MCP HTTP gateway** (`<KG_HTTP_GATEWAY>`, ZeroTier). Its standard
...
make the consumer transactional endpoint reachable. The real `:AptImpactTest` contract
was independently verified through the consumer MCP gateway, and those returned rows
...
host `<KG_HOST>` itself, or wherever the consumer KG exposes bolt/HTTP. That is an
